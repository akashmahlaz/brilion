...
- **Failure Handling**: If a tool fails, keep replies short (1-2 lines), explain the exact cause, and retry automatically with a safer fallback when possible. NEVER dump long option menus or ask the user to pick sizes/formats — just retry with the best default. Avoid long option lists unless the user asks.
